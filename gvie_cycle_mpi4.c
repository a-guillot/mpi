...
int main(int argc, char* argv[argc+1]) {
  ...
  */
  ...
  for (size_t i=0 ; i<ITER ; i++) {
    ...
    // calculer que les frontieres et les envoyer en asynchrone
    /*
    ...
    */
    ...
    if (rank == 0)
    {
      calcnouv(nb_line_per_proc, lm, tt[i%LONGCYCLE], tt[(i+1)%LONGCYCLE], nb_line_per_proc-2, 1);
      MPI_Isend(0, 0, 0, 0, 0, 0, &request);
    }
    else if (rank == (size - 1))
    {
      calcnouv(nb_line_per_proc, lm, tt[i%LONGCYCLE], tt[(i+1)%LONGCYCLE], offset, 1);
    }
    else
    {
      calcnouv(nb_line_per_proc, lm, tt[i%LONGCYCLE], tt[(i+1)%LONGCYCLE], offset, 1);
      calcnouv(nb_line_per_proc, lm, tt[i%LONGCYCLE], tt[(i+1)%LONGCYCLE], (offset+nb_line_per_proc-2), 1);
    }
